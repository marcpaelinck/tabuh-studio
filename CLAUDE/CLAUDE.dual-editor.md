...
- **`SystemNode` branch.** In **compact** view it renders only the `CompactSystemEditor`; in **expanded** view it renders only the read-only per-position notation (the positions textarea + `SystemNotationViewer readOnly`, which also carries the playback highlight). The header (`PlaybackButtons` + `SummaryItem`s) is shown in both; the `SummaryItem`s get `disabled={editorView === 'expanded'}`.
- **Shared `StaffGrid` surface.** Both views render through one presentational component, `StaffGrid` (`components/editor/StaffGrid.tsx`). It draws a focusable container holding a single background grid element behind a stack of `StaffLine` rows. The caller supplies `rows: StaffGridRow[]` (each row = `symbols` + `cursorIndex` + click handlers, plus an optional `label` element and an optional `below` block) and the keyboard/focus handlers; all the layered-grid + `StaffLine` plumbing lives in `StaffGrid`, so the two views stay in sync and differ only in how they build their rows.
    - `CompactSystemEditor` builds one row per group (label = the click-popover group chip; `below` = the expansion snippet) and passes `grid={{ ref, left: '9rem', widthCh: maxCols }}` so the grid is an **inner, label-offset** element; the paint ref (`compactNotationRef`) flows only to `grid.ref`, so the grid aligns with the notation rather than the label column. It does **not** pass `ref` to the outer container.
    - `SystemNotationViewer` builds one row per staff with no label and **omits** `grid`, so its container is a transparent overlay on top of the playback textarea; its `ref` (`expandedNotationRef`) is the outer element and the gridlines/highlight come from the textarea behind it. `StaffGrid`'s `ref` prop is therefore optional.
    - Grid painting: `SystemNode`'s "redraw background gridlines" effect paints `compactNotationRef` (via `createGridStyle`, compact colours) and `expandedNotationRef` (expanded colours), including the moving playback-cursor band; `editorView` is in the deps so the active view's grid repaints on switch.
    - Stacking: `StaffGrid` layers the grid (`z-0`) beneath the rows (`relative z-10`) inside a `relative` box. The `EditorWindow` view-toggle bar therefore uses `z-30` so toggle clicks win over overlapping compact position labels (raising the `SegmentedControl`'s own z-index has no effect — it's nested inside the bar's stacking context).
