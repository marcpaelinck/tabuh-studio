...
import { WpApiFunctions } from './contexts'
import {
    Dashboard,
...
export function MainWindow({ dataSource }: MainWindowProps) {
    ...
    const wpFunc = useContext(WpApiFunctions) // DELETE

    const [expanded, setExpanded] = useState<Record<string, boolean>>({}) // KEEP
    const [buttonIsExpand, setButtonIsExpand] = useState<boolean>(!editorInitialExpandState) // KEEP
    const [keyboard, SetKeyboard] = useState<KeyboardType>('regular') // KEEP

    // PLAYBACK SETTINGS
    const [focus, setFocus] = useState<Position[]>([]) // DELETE
    const {
        playbackFunctions,
        ...
        schedulePlayback,
        totalDurationMs
    } = usePlaybackManager(focus) // DELETE
    const playbackReducer = playbackReducerFactory(playbackFunctions, schedulePlayback) // DELETE
    ...
    useEffect(() => updatePlaybackFunctions({ generic: stopPlayback }), []) // DELETE
    ...
    const ToggleIcon = sidenavExpanded ? ArrowRightLineIcon : ArrowLeftLineIcon //KEEP
    ...
    return (
        <Container id="main" height="80vh">
            <Container id="header+content" className="flex w-full">
                ...
                <Content id="content" px="1rem" className="h-9/10"></Content>
            </Container>
