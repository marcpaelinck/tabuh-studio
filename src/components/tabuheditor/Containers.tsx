...
export function SectionText({
    ...
    return (
        <g id={`section-${id}`}>
            <g id={`tags-${id}`}></g>
            <g id={`staves-${id}`}>
                {Object.entries(section.staves)
                    .toSorted(([p1, _1], [p2, _2]) => editorSortingOrder.indexOf(p1) - editorSortingOrder.indexOf(p2))
                    .map(([position, stave], idx) => {
                            ...
                            <>
                                {section.id == 1 && show && (
                                    <TextContainer
                                        ...
                                        x={0}
                                        y={row + idx}
                                        content={[positionConfigs[position].name]}
                                        editable={false}
    ...
    return (
        <g id={`te-score}`} className="flex display-scroll">
            {score.systems.map((system, idx) => {
                const row = curRow;
                curRow +=
                    Math.max(...system.sections.map((section) => Object.entries(section.staves).length)) +
                    rowsBetweenSystems;
                return (
                    <SystemText
                        key={idx}
