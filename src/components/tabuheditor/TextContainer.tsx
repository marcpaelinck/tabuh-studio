import { useMemo, useRef, type Dispatch, type HTMLAttributes} from "react"
import type { JsonSymbol, TextCursorPosition } from "../../models/types"

export function FontSymbol({symbol, ...props} : {symbol: JsonSymbol | string} & HTMLAttributes<HTMLSpanElement>) {

    function moveCursor(event: React.MouseEvent<HTMLSpanElement, MouseEvent>) {
        // TBD
    }

    return (
        <>
            <span className={props.className} onClick={(e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => moveCursor(e)}>
...
export function TextContainer({id, x, y, content, editable, setCursorPosition, ...props}:
    {id: string, x: number, y: number, content: JsonSymbol[] | string[], editable: boolean, setCursorPosition: Dispatch<TextCursorPosition>} & Omit<HTMLAttributes<HTMLSpanElement>, "id">) {
    ...
    return(
        <>
            ...
            <div id={id} className={`text-[${fs}pt] ` + (editable ? "balifont" : "font-sans")} >{textContent}</div>
        </>
    )
