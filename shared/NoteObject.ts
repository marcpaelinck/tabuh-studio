...
export class NoteObject {
  ...
  constructor(input: string, position?: Position, fault?: NoteObjectFault) {
    ...
    try {
      NoteObject.validate(input);

      for (const ch of input) {
        if (!pitch && GRACE_NOTE_PREFIXES.has(ch)) {
          prefix = ch;
        } else if (!pitch && PITCH_CHARS.has(ch)) {
          pitch = ch;
        } else if (pitch && OCTAVE_MODIFIERS.has(ch)) {
          octave = ch as "," | "<";
        } else if (pitch && STROKE_MODIFIERS.has(ch)) {
          stroke = ch;
        } else if (pitch && PATTERN_MODIFIERS.has(ch)) {
          pattern = ch;
        }
      }
    } catch {
  ...
  static validate(symbol: string, position?: Position): void {
    ...
    for (const ch of symbol) {
      if (!hasPitch && GRACE_NOTE_PREFIXES.has(ch)) {
        prefixCount++;
        if (prefixCount > 1)
          throw new NoteObjectError("multiple grace note prefixes", symbol);
      } else if (!hasPitch && PITCH_CHARS.has(ch)) {
        hasPitch = true;
      } else if (hasPitch && OCTAVE_MODIFIERS.has(ch)) {
        octaveCount++;
        if (octaveCount > 1)
          throw new NoteObjectError("multiple octave modifiers", symbol);
      } else if (hasPitch && STROKE_MODIFIERS.has(ch)) {
        strokeCount++;
        if (strokeCount > 1)
          throw new NoteObjectError("multiple stroke modifiers", symbol);
      } else if (hasPitch && PATTERN_MODIFIERS.has(ch)) {
        patternCount++;
        if (patternCount > 1)
          throw new NoteObjectError("multiple pattern modifiers", symbol);
      } else {
    ...
    if (!hasPitch)
      throw new NoteObjectError("no pitch character found", symbol);
  ...
  static parseText(text: string, position?: Position): NoteObject[] {
    ...
    for (const ch of text) {
      const startsNewSymbol =
        hasPitch && (PITCH_CHARS.has(ch) || GRACE_NOTE_PREFIXES.has(ch));
      if (startsNewSymbol) {
